...
class testSlapOSAbyss(SlapOSTestCaseMixin):
  ...
  def afterSetUp(self):
    ...
    for compute_node_id in ('database_debian10', 'database_debian11', 'node_debian10'):
  ...
  def beforeTearDown(self):
    ...
    for compute_node in self.portal.portal_catalog(portal_type='Compute Node', reference=('database_debian10', 'database_debian11', 'node_debian10')):
...
{"end_date": "2022/11/15 17:08 CET", "end_marker": "fluentbit_end"}\n'
  ...
  def _test_data_transformation_with_exclude_path(self):
    request = self.portal.REQUEST
    request_dict = self._create_request_dict()
    ...
    self.portal.portal_alarms.wendelin_handle_analysis.activeSense()
    self.tic()
    data_array = database_debian11.getCausalityRelatedValue(portal_type='Data Array')
    ...
    self.portal.portal_alarms.wendelin_handle_analysis.activeSense()
    self.tic()
    for new_array in database_debian11.getCausalityRelatedValueList(portal_type='Data Array'):
  ...
  def test_compare_data_array(self):
    ...
    ## Call DataArray_processDataArray
  ...
  def test_compute_node_modified_file(self):
    ...
    ## Create Data Arrays
    # Node servers
    node_debian10 = self.portal.compute_node_module['node_debian10']
    tmp_data_array = self.portal.data_array_module.newContent(portal_type='Data Array', title='%s-%s' % (server_publication_list[0],server_publication_list[1]))
    tmp_data_array.setCausality('compute_node_module/node_debian10')
    ...
    reference_data_array_list = []
    for index, uid_ndarray in enumerate(reference_uid_ndarray_list):
      tmp_data_array = self.portal.data_array_module.newContent(portal_type='Data Array', title='%s-%s' % (reference_publication_section_list_list[index][0], reference_publication_section_list_list[index][1]))
      tmp_data_array.setCausality('compute_node_module/database_debian10')
      tmp_data_array.initArray(shape=(uid_ndarray.size,), dtype='int64')
      tmp_data_array.setArray(uid_ndarray)
      tmp_data_array.setPublicationSectionList(reference_publication_section_list_list[index])
      reference_data_array_list.append(tmp_data_array)
    ...
    self.portal.portal_alarms.slapos_process_data_array.activeSense()
    self.tic()
    self.assertTrue(node_debian10.ComputeNode_hasModifiedFile() is None)
    ...
    self.portal.portal_alarms.slapos_process_data_array.activeSense()
    self.tic()
    self.assertTrue(node_debian10.ComputeNode_hasModifiedFile() is not None)
  ...
  def test_whole_process_from_data_stream_to_data_array_comparaison(self):
    ...
    for reference in request_dict:
      compute_node = self.portal.portal_catalog.getResultValue(
        portal_type='Compute Node',
        reference = reference)
      data_array = compute_node.getCausalityRelatedValue(portal_type='Data Array')
      self.assertEqual(data_array.getPublicationSectionList(), compute_node.getPublicationSectionList())
  ...
  def test_multi_data_array_when_have_multi_data(self):
    ...
    self.tic()
    new_data_array_list = self.portal.portal_catalog(portal_type='Data Array')
    self.assertEqual(len(new_data_array_list), 6)
    self.tic()
