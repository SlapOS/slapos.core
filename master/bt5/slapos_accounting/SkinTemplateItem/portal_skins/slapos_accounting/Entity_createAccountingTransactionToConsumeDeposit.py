...
if create_credit_note:
  ...
  for invoice in invoice_list:
    if invoice.total_price and (not invoice.SaleInvoiceTransaction_isLettered()):
      # No deposit remaining. Create full invoice mirror
      invoice.SaleInvoiceTransaction_createReversalSaleInvoiceTransaction(batch_mode=1)
      assert invoice.SaleInvoiceTransaction_isLettered()
...
for line in invoice_paid_line_list + deposit_use_line_list:
  assert line.hasGroupingReference(), line.getRelativeUrl()
