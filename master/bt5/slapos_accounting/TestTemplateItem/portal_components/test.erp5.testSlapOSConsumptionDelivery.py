...
class TestSlapOSComputerConsumptionTioXMLFile_solveInvoicingGeneration(
  ...
  def bootstrapGenerateConsumptionDeliveryTest(self, shared=False):
    with PinnedDateTime(self, DateTime('2024/02/20')):
      currency, seller_organisation, _, person, _ = self.bootstrapVirtualMasterTest(
      ...
      project = self.addDefaultProject(person=person, currency=currency)
      self.tic()

    with PinnedDateTime(self, DateTime('2024/02/20 01:01')):
      self.login(person.getUserId())

      compute_node = self.requestComputeNode(
    ...
    instance_tree = self._getCurrentInstanceTreeList(title=instance_title)[0].getObject()
    another_instance_title = self._getCurrentInstanceTreeList(title=another_instance_title)[0].getObject()

    return seller_organisation, compute_node, instance_tree, another_instance_title, service
  ...
  def generateConsumptionDelivery(self, person, document, tio_dict):
    ...
    self.assertEqual(delivery.getSourceProject(), None)
