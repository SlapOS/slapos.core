...
tag = 'script_Base_triggerCreateConsumptionSubscriptionSimulation'
...
if (start_date is not None) and (start_date < current_date):
  for open_order_line in open_sale_order.objectValues():
    for ob in [open_order_line] + open_order_line.getCellValueList():
      for item in ob.getAggregateValueList(portal_type='Consumption Subscription'):
        ob.reindexObject(activate_kw={'tag': tag})
        item.activate(after_tag=tag, activity='SQLQueue').Base_reindexAndSenseAlarm(['slapos_accounting_create_consumption_subscription_simulation'])
