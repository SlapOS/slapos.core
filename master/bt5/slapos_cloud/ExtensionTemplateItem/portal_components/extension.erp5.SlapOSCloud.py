...
def SoftwareInstance_renameAndStop(self, REQUEST=None):
  ...
  promise_kw = {
      'instance_xml': self.getTextContent(),
      # change software type, as this will minimise the chance of the instance
      # content being processed by the node, and that's the expected situation
      'software_type': self.getSourceReference() + suffix,
      'sla_xml': self.getSlaXml(),
      'software_release': self.getUrlString(),
      'shared': self.getPortalType()=="Slave Instance",
  }
