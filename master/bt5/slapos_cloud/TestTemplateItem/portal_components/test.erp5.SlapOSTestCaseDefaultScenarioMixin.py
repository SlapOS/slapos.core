...
class DefaultScenarioMixin(TestSlapOSSecurityMixin):
  ...
  def requestComputeNode(self, title):
    requestXml = self.portal.portal_slap.requestComputeNode(title)
    self.tic()
  ...
  def formatComputeNode(self, compute_node, partition_count=10):
    ...
    try:
      ...
      self.portal.portal_slap.loadComputeNodeConfigurationFromXML(
  ...
  def simulateSlapgridSR(self, compute_node):
    ...
    try:
      ...
      compute_node_xml = self.portal.portal_slap.getFullComputeNodeInformation(
    ...
    try:
      ...
      for partition in slap_compute_node._computer_partition_list:
        if partition._requested_state == 'destroyed' \
          ...
          self.portal.portal_slap.destroyedComputePartition(compute_node.getReference(),
      ...
      for partition in slap_compute_node._computer_partition_list:
        if partition._requested_state in ('started', 'stopped') \
          ...
          try:
            ...
            self.portal.portal_slap.setComputePartitionConnectionXml(
              compute_node_id=compute_node_reference,
              compute_partition_id=partition._partition_id,
  ...
  def personRequestInstanceNotReady(self, **kw):
    response = self.portal.portal_slap.requestComputePartition(**kw)
