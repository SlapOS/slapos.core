...
class TestSlapOSPropagateRemoteNodeInstance(SlapOSTestCaseMixin):
  ...
  def test_propagateRemoteNode_script_propageRemoteNodeSlaToRemoteInstanceTree(self):
    instance_tree = self.addInstanceTree()
    software_instance = instance_tree.getSuccessorValue()
    ...
    self.assertEqual(remote_instance_tree.getTextContent(),
                     software_instance.getTextContent())
    self.assertEqual(remote_instance_tree.getSlaXml(), remote_node.getSlaXml())
    self.assertEqual(remote_instance_tree.isRootSlave(False),
                     software_instance.getPortalType() == 'Slave Instance')

    self.assertEqual(software_instance.getConnectionXml(), None)

  def test_propagateRemoteNode_script_doNotPropageRemoteNodeBrokenSla(self):
    instance_tree = self.addInstanceTree()
    software_instance = instance_tree.getSuccessorValue()
...
class TestERP5GarbageCollectInvalidatedNodeFromAllocationSupplyAlarm(SlapOSTestCaseMixin):
  ...
  def test_garbageCollectInvalidatedNodeFromAllocationSupply_script_allToRemove(self):
    ...
    self.assertEqual(allocation_supply.getValidationState(), 'invalidated')
