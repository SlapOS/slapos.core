...
class TestRegularisationRequest_getInvoiceList(SlapOSTestCaseMixin):
  ...
  def test_two_users_invoice_isolation(self):
    project = self.addProject()
    person_a = self.makePerson(project, user=1)
    person_b = self.makePerson(project, user=1)

    # Create invoices for each person with different dates
    ...
    # Create regularisation requests
    ...
    # Person A sees only their invoices, sorted by descending date
    result_a = reg_a.RegularisationRequest_getInvoiceList()
    self.assertEqual(len(result_a), 2)
    self.assertEqual(result_a[0].uid, invoice_a1.getUid())  # 2024/06 first
    self.assertEqual(result_a[1].uid, invoice_a2.getUid())  # 2024/01 second
  ...
  def test_only_automated_ledger(self):
    project = self.addProject()
    person = self.makePerson(project, user=1)
  ...
  def test_only_stopped_delivered(self):
    project = self.addProject()
    person = self.makePerson(project, user=1)

    stopped_invoice = self.createFinalInvoice(person, is_stopped=True)
    self.createFinalInvoice(person, is_stopped=False)  # confirmed, not stopped
