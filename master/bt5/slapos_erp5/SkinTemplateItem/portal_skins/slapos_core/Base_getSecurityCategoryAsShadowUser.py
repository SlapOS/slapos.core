...
actor_list = [i for i in obj.getValueList(base_category)
                 if i.getPortalType() in obj.getPortalObject().getPortalActorTypeList()]

if not actor_list:
  return []

# The assignee is hardcoded here.
actor_shadow_id = "SHADOW-%s" % actor_list[0].getUserId()

# Return all usecased so the role filter on upper level
return {"Assignee": [actor_shadow_id],
        "Auditor": [actor_shadow_id]}
