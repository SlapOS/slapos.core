...
def ComputeNode_simulateSlapgridCP(self, instance_connection_dict=None,
  ...
  try:
    ...
    compute_node_xml = portal.portal_slap.getFullComputeNodeInformation(
    ...
    assert 'Compute Node' == slap_compute_node.__class__.__name__
    ...
    for partition in slap_compute_node._computer_partition_list:
      if partition._requested_state in ('started', 'stopped') \
        ...
        portal.portal_slap.setComputePartitionConnectionXml(
...
def ComputeNode_simulateSlapgridFormat(self, partition_count=10):
  ...
  try:
    ...
    return portal.portal_slap.loadComputeNodeConfigurationFromXML(
