...
class TestMetadataAgent(SlapOSTestCaseMixin):

  def afterSetUp(self):
    super(TestMetadataAgent, self).afterSetUp()
    self.tic()
    self.portal.portal_catalog.erp5_mysql_innodb.z0_drop_data_analysis()
    self.portal.portal_catalog.erp5_mysql_innodb.z_create_data_analysis()
    self.tic()
...
tail.0:2021-10-15 15:11:02.230745474 +0200 CEST[fluentbit_end]\n'
  ...
  def string_to_ndarray(self, string):
    data_stream = self.portal.data_stream_module.newContent(portal_type='Data Stream', reference='TMP.string_to_ndarray')
    # NOTE: code from Metadata_convertDataStreamToArray
    # from get_end_and_json_list()
  ...
  def test_compare_data_array(self):
    ...
    for l in server_uid_list_list:
      l.append(-1) # each uid_list should be ending with -1
    ...
    # each uid_list should be ending with -1
    reference_uid_list_list = [
      [i for i in range(20, 30)],
      [i for i in range(5, 15)]
    ]
    for l in reference_uid_list_list:
      l.append(-1)
  ...
  def test_data_product_create_validate_data_supply(self):
    self.data_product.DataProduct_createDataSupply(batch=1)
    self.tic()
    request = self.portal.REQUEST
