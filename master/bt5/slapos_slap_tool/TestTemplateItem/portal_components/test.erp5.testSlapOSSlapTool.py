...
class TestSlapOSSlapToolgetFullComputeNodeInformation(TestSlapOSSlapToolMixin):
  def test_activate_getFullComputeNodeInformation_first_access(self):
    ...
    response = self.portal_slap.getFullComputeNodeInformation(self.compute_node_id)
    self.commit()
...
class TestSlapOSSlapToolComputeNodeAccess(TestSlapOSSlapToolMixin):
  def test_getFullComputeNodeInformation(self):
    self._makeComplexComputeNode(with_slave=True)
...
""" % dict(
  ...
  def test_getComputeNodeInformation(self):
    self.assertEqual('getFullComputeNodeInformation',
      self.portal_slap.getComputeNodeInformation.im_func.func_name)

  def test_not_accessed_getComputeNodeStatus(self):
    self.login(self.compute_node_user_id)
    created_at = rfc1123_date(DateTime())
    since = created_at
    response = self.portal_slap.getComputeNodeStatus(self.compute_node_id)
    self.assertEqual(200, response.status)
...
""" % dict(
  ...
  def test_accessed_getComputeNodeStatus(self):
    self.login(self.compute_node_user_id)

    self.portal_slap.getFullComputeNodeInformation(self.compute_node_id)
    created_at = rfc1123_date(DateTime())
    since = created_at
    response = self.portal_slap.getComputeNodeStatus(self.compute_node_id)
    self.assertEqual(200, response.status)
...
""" % dict(
  ...
  def test_loadComputeNodeConfigurationFromXML(self):
...
""" % {'compute_node_reference': self.compute_node.getReference()}
      ...
      response = self.portal_slap.loadComputeNodeConfigurationFromXML(
        compute_node_xml)
...
""" % dict(
  ...
  def test_useComputeNode_wrong_xml(self):
    self.login(self.compute_node_user_id)
    response = self.portal_slap.useComputeNode(
  ...
  def test_useComputeNode_expected_xml(self):
...
<category>None</category>
  ...
  def test_useComputeNode_empty_reference(self):
    self.compute_node_use_compute_node_simulator = tempfile.mkstemp()[1]
    try:
      self.login(self.compute_node_user_id)
      self.compute_node.ComputeNode_reportComputeNodeConsumption = Simulator(
...
class TestSlapOSSlapToolInstanceAccess(TestSlapOSSlapToolMixin):
  def test_getComputePartitionCertificate(self):
    self._makeComplexComputeNode()
    ...
    response = self.portal_slap.getComputePartitionCertificate(self.compute_node_id,
        partition_id)
    self.assertEqual(200, response.status)
...
""" % dict(
  ...
  def test_getComputePartitionStatus(self):
    self._makeComplexComputeNode()
    partition_id = self.start_requested_software_instance.getAggregateValue(
        portal_type='Compute Partition').getReference()
    created_at = rfc1123_date(DateTime())
    since = created_at
    self.login(self.start_requested_software_instance.getUserId())
    response = self.portal_slap.getComputePartitionStatus(self.compute_node_id,
      partition_id)
    self.assertEqual(200, response.status)
    self.assertEqual( 'public, max-age=60, stale-if-error=604800',
        response.headers.get('cache-control'))
...
""" % dict(
  ...
  def test_getComputePartitionStatus_visited(self):
    self._makeComplexComputeNode()
    partition_id = self.start_requested_software_instance.getAggregateValue(
        portal_type='Compute Partition').getReference()
    created_at = rfc1123_date(DateTime())
    since = created_at
    self.login(self.start_requested_software_instance.getUserId())
    self.portal_slap.registerComputePartition(self.compute_node_id, partition_id)
    response = self.portal_slap.getComputePartitionStatus(self.compute_node_id,
      partition_id)
    self.assertEqual(200, response.status)
    self.assertEqual( 'public, max-age=60, stale-if-error=604800',
...
""" % dict(
  ...
  def test_registerComputePartition_withSlave(self):
    self._makeComplexComputeNode(with_slave=True)
    partition_id = self.start_requested_software_instance.getAggregateValue(
        portal_type='Compute Partition').getReference()
    self.login(self.start_requested_software_instance.getUserId())
    response = self.portal_slap.registerComputePartition(self.compute_node_id, partition_id)
    self.assertEqual(200, response.status)
    self.assertEqual( 'public, max-age=1, stale-if-error=604800',
        response.headers.get('cache-control'))
    self.assertEqual('REMOTE_USER',
...
""" % dict(
  ...
  def test_registerComputePartition(self):
    self._makeComplexComputeNode()
    partition_id = self.start_requested_software_instance.getAggregateValue(
        portal_type='Compute Partition').getReference()
    self.login(self.start_requested_software_instance.getUserId())
...
"""
    ...
    try:
      ...
      response = self.portal_slap.setComputePartitionConnectionXml(
...
""" % dict(
  ...
  def test_destroyedComputePartition(self):
    ...
    response = self.portal_slap.destroyedComputePartition(self.compute_node_id,
      partition_id)
    self.assertEqual('None', response)
  ...
  def test_request_withSlave(self):
    ...
    try:
      ...
      response = self.portal_slap.requestComputePartition(
          compute_node_id=self.compute_node_id,
...
<marshal>
  <list id="i2"><string>Instance0</string><string>Instance1</string></list>
    ...
    self.portal_slap.updateComputePartitionRelatedInstanceList(
        compute_node_id=self.compute_node_id,
        compute_partition_id=partition_id,
...
<list id="i2"><string>instance0</string></list>
  ...
  def test_stoppedComputePartition(self):
    ...
    partition_id = self.start_requested_software_instance.getAggregateValue(
        portal_type='Compute Partition').getReference()
    self.login(self.start_requested_software_instance.getUserId())
    response = self.portal_slap.stoppedComputePartition(self.compute_node_id,
      partition_id)
    self.assertEqual('None', response)
    created_at = rfc1123_date(DateTime())
    since = created_at
...
""" % dict(
  ...
  def test_startedComputePartition(self):
    ...
    partition_id = self.start_requested_software_instance.getAggregateValue(
        portal_type='Compute Partition').getReference()
    self.login(self.start_requested_software_instance.getUserId())
    response = self.portal_slap.startedComputePartition(self.compute_node_id,
      partition_id)
    self.assertEqual('None', response)
    created_at = rfc1123_date(DateTime())
    since = created_at
...
""" % dict(
  ...
  def test_requestComputeNode(self):
    ...
    try:
      ...
      response = self.portal_slap.requestComputeNode(compute_node_id)
...
""" % {'compute_node_id': compute_node_reference}
  ...
  def assertGenerateComputeNodeCertificateSimulator(self, args, kwargs):
    ...
    self.assertEqual(stored,
      [{'recargs': args, 'reckwargs': kwargs,
      'recmethod': 'generateComputeNodeCertificate'}])

  def test_generateComputeNodeCertificate(self):
    ...
    try:
      ...
      response = self.portal_slap.generateComputeNodeCertificate(self.compute_node_id)
...
""" % {'compute_node_key': compute_node_key, 'compute_node_certificate': compute_node_certificate}
  ...
  def assertRevokeComputeNodeCertificateSimulator(self, args, kwargs):
    ...
    self.assertEqual(stored,
      [{'recargs': args, 'reckwargs': kwargs,
      'recmethod': 'revokeComputeNodeCertificate'}])

  def test_revokeComputeNodeCertificate(self):
    ...
    try:
      ...
      response = self.portal_slap.revokeComputeNodeCertificate(self.compute_node_id)
      self.assertEqual('None', response)
