...
class Bouture(object):
    ...
    def partition_path(partition_id, *fragments):
        return os.path.join(self.instance_root, partition_id, *fragments)

    def find_bouture_partitions(self):
        bouture_partitions = []
        base = self.slappart_base
        for p in os.listdir(self.instance_root):
            if p.startswith(base) and p[base:].isdigit(): # p is a partition
    ...
    def graft(self, args):
        ...
        partitions = self.find_bouture_partitions()
        if not partitions:
            self.logger.info(
        ...
        for partition_id in partitions:
            ...
            # need to be reconfigured to bang the new master, and possibly
