...
class Interface(object):
  ...
  def getAddress(self, pref_addr=None):
    ...
    If pref_addr is given, return the lowest interface address whose network
    contains pref_addr (works whether pref_addr is the computer's own address
    or a partition address derived from it).  Falls back to the address with
    the largest subnetwork (smallest masklen).
    """
      ...
      for a in address_list:
        try:
          if netaddr.IPAddress(pref_addr) in netaddr.IPNetwork(
            ...
            suitable_addr_list.append(a)
        except (netaddr.AddrFormatError, ValueError):
          pass
      if suitable_addr_list:
        return min(suitable_addr_list, key=lambda a: netaddr.IPAddress(a['addr']))
    return min(address_list, key=lambda a: (lenNetmaskIpv6(a['netmask']),netaddr.IPAddress(a['addr'])))
