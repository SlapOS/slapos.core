...
class Interface(object):
  ...
  def addIPv6Address(self, partition_index, addr=None, netmask=None, tap=None):
    ...
    if not tap or self._reserveIpv6Range(result_addr['addr'], result_addr['prefixlen']):
    ...
    for _ in range(10):
      ...
      if not tap or self._reserveIpv6Range(addr, result_addr['prefixlen']):
  ...
  def generateIPv6Range(self, i):
    ...
    if self._reserveIpv6Range(ipv6_range['addr'], ipv6_range['prefixlen']):
      return ipv6_range
    # Try 10 times to add address, raise in case if not possible
    for _ in range(10):
      result_addr = self._generateRandomIPv6Range(address_dict, suffix='0')
      if self._reserveIpv6Range(address, ipv6_range['prefixlen']):
        return result_addr
