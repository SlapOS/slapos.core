...
LEGACY_SNAPSHOT_PATH = os.path.join('.slapgrid', 'functionality-snapshot.json')

INSTANCE_ROOT_ENVIRONMENT_NAME = 'SLAPGRID_INSTANCE_ROOT'

STATUS_TAG = '[functionality-locked]'

# Kept so a partition locked before the per-call store existed does not have to
# be unlocked to become lockable again.
LEGACY_FIELD_CALL_LIST = (
...
def seedFromLegacySnapshot(partition_path):
  """Fill an empty store from the pre-per-call snapshot, if one is left."""
  if os.path.isdir(os.path.join(partition_path, STORE_PATH)):
    return
  try:
    with open(os.path.join(partition_path, LEGACY_SNAPSHOT_PATH)) as f:
    ...
    return
  for field, method, args in LEGACY_FIELD_CALL_LIST:
