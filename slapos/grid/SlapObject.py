...
class Partition(object):
  ...
  def start(self):
    ...
    partition_id = self.computer_partition.getId()
    try:
      with self.getSupervisorRPC() as supervisor_rpc:
        supervisor_rpc.supervisor.startProcessGroup(partition_id, False)
  ...
  def stop(self):
    ...
    try:
      with self.getSupervisorRPC() as supervisor_rpc:
        supervisor_rpc.supervisor.stopProcessGroup(partition_id, False)
    except xmlrpclib.Fault as exc:
      if exc.faultString.startswith('BAD_NAME:'):
  ...
  def checkProcessesFromStateList(self, process_list, state_list):
    ...
    for process in process_list:
      try:
        with self.getSupervisorRPC() as supervisor_rpc:
          info = supervisor_rpc.supervisor.getProcessInfo(process)
