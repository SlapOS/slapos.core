...
class Software(object):
  ...
  def _copy_netrc_file(self, dest_path):
    if not os.getuid():
      # if root, home_path is the root folder of software_root owner
      home_path = pwd.getpwuid(os.stat(self.software_root).st_uid).pw_dir
    else:
      # if not root, we rely on what is in $HOME
      home_path = os.environ['HOME']
    home_netrc = os.path.join(home_path, '.netrc')
