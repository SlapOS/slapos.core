...
def create_slapgrid_object(options, logger):
  ...
  return Slapgrid(software_root=op['software_root'],
                  ...
                  config=options,
                  setup_only=op.get('setup_only', False))
...
""" %  {'dbus_executable': self.firewall_conf['dbus_executable'],
  ...
  def processComputerPartition(self, computer_partition):
    ...
    computer_partition_state = computer_partition.getState()
    action = computer_partition_action.get(computer_partition_state)
    if self.setup_only and action in ('start', 'stop'):
      action = 'setup-only'
    ...
    if timestamp and not self.setup_only:
