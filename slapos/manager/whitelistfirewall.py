...
class Manager(object):
  ...
  whitelist_firewall_timestamp = '.slapos-whitelist-firewall-timestamp'
  ...
  def instanceTearDown(self, partition):
    ...
    whitelist_firewall_timestamp_path = os.path.join(
      partition.instance_path, self.whitelist_firewall_timestamp)
    ...
    if not os.path.exists(whitelist_firewall_path):
      ...
      if fwCommunicate(['--query-chain'] + chain_cmd).strip() == 'yes':
        fwCommunicate(['--remove-rules', 'ipv4', 'filter', chain_name])
        fwCommunicate(['--remove-chain'] + chain_cmd)
        reload_firewall = True
      rule_cmd = [
        'ipv4', 'filter', 'OUTPUT', '0', '-m', 'owner', '--uid-owner',
        str(user_id), '-j', chain_name]
      if fwCommunicate(['--query-rule'] + rule_cmd).strip() == 'yes':
        fwCommunicate(['--remove-rule'] + rule_cmd)
        reload_firewall = True
      if reload_firewall:
        reloadFirewalld()
        logger.info(
      ...
      if os.path.exists(whitelist_firewall_timestamp_path):
        os.unlink(whitelist_firewall_timestamp_path)
      return

    whitelist_firewall_path_timestamp = int(os.path.getmtime(
      whitelist_firewall_path))
    # move back one second to accommodate to eventual rounding
    whitelist_firewall_path_timestamp -= 1
    ...
    try:
      with open(whitelist_firewall_timestamp_path, 'rb') as fh:
        previous_timestamp = int(fh.read())
    except Exception:
      # whatever happened, it means that timestamp became unreadable, so
      # simply reset previous timestamp
    ...
    if fwCommunicate(['--query-chain'] + chain_cmd).strip() != 'yes':
      ...
      chain_added = True
    if chain_added or whitelist_firewall_path_timestamp > previous_timestamp:
      with open(whitelist_firewall_timestamp_path, 'wb') as fh:
      ...
      reloadFirewalld()
      with open(whitelist_firewall_timestamp_path, 'wb') as fh:
        # "commit" changes to the timestamp file
        fh.write('%i' % (whitelist_firewall_path_timestamp,))
