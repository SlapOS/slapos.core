...
class Manager(object):
  ...
  def instanceTearDown(self, partition):
    ...
    # whitelist port 53/udp, the resolver XXX - seems too much!
    fwCommunicate(['--add-rule', 'ipv4', 'filter', chain_name, '0', '-p', 'udp', '--dport', '53', '-j', 'ACCEPT'])
    # drop everything else
    fwCommunicate(['--add-rule', 'ipv4', 'filter', chain_name, '0', '-j', 'REJECT'])
    # configure the rule
    rule_cmd = ['ipv4', 'filter', 'OUTPUT', '0', '-m', 'owner', '--uid-owner', str(user_id), '-j', chain_name]
