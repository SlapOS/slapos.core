...
def busy_root_shared_list(title=None):
  ...
  for row in execute_db('slave', query, args):
    ...
    s['title'] = row['reference'][1:]
    s['relative_url'] = url_for('hateoas_shared', slave_reference=row['reference'])
    shared.append(s)
  return shared
...
@app.route('/hateoas/shared/<slave_reference>', methods=['GET'])
def hateoas_shared(slave_reference):
  ...
  return hateoas_service_document(
    reference = slave_reference[1:], # root shared are prefixed with _
