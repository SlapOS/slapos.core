...
def busy_partitions_list(title):
  partitions = []
...
def hateoas_root():
  return {
    '_links': {
      'raw_search': {
      'href': request.host_url[:-1] + unquoted_url_for('hateoas', mode='search', query='{query}', select_list='{select_list}')
    },
      'traverse': {
        'href': request.host_url[:-1] + unquoted_url_for('hateoas', mode='traverse', relative_url='{relative_url}', view='{view}')
      },
    }
  }
