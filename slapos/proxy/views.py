...
def softwareInstanceBang():
  requested_by = getRequesterFromForm(request.form)
  execute_db('partition',
    'UPDATE %s SET timestamp=?'
    ' WHERE (partition_reference=? AND requested_by IS NULL) OR requested_by=?',
...
def requestComputerPartition():
  form = request.form
  parsed_request_dict = {
    'requested_by': getRequesterFromForm(form),
  ...
  if slave:
    ...
    slave_reference = (parsed_request_dict['requested_by'] or ''
                      ) + '_' + parsed_request_dict['partition_reference']
...
def getAllocatedInstance(partition_reference, requested_by=None):
  """
  Look for existence of instance, if so return the
  corresponding partition dict, else return None
  """
  a = [partition_reference]
  q = 'SELECT * FROM %s WHERE partition_reference=? AND requested_by'
  if requested_by is None:
    q += ' IS NULL'
  else:
    q += '=?'
    a.append(requested_by)
  return execute_db('partition', q, a, one=True)
...
def requestSlave(software_release, software_type, partition_reference, requested_by, partition_parameter_kw, filter_kw, requested_state):
  ...
  new_slave = {}
  slave_reference = (requested_by or '') + '_' + partition_reference
...
def busy_root_shared_list(title=None):
  shared = []
  query = 'SELECT * FROM %s WHERE asked_by IS NULL'
  args = []
  if title:
    query += ' AND reference=?'
    args.append('_' + title)
