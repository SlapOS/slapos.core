...
# We only need to handle the hatoeas requests made by
#   slapos service list
...
def partition_list(title):
  ...
  for row in execute_db('partition', query, args):
    ...
    p['relative_url'] = url_for('hatoeas_partitions', partition_reference=p['partition_reference'])
    partitions.append(p)
  return partitions

def computer_list(reference):
  computers = []
  query = 'SELECT * FROM %s'
  args = []
  if reference:
  ...
  for row in execute_db('computer', query, args):
    c = dict(row)
    c['title'] = c['reference']
    c['relative_url'] = url_for('hatoeas_computers', computer_reference=c['reference'])
    computers.append(c)
  return computers

def parse_query(query):
  query_fragments = [q.strip().split(':') for q in query.split('AND')]
  if not all(len(q) == 2 for q in query_fragments):
    return None
  query_map = dict(query_fragments)
  portal_type = query_map.get('portal_type', '').strip('"')
  if portal_type == 'Hosting Subscription':
    title = query_map.get('title', '').lstrip("=").strip('"')
    return partition_list(title)
  elif portal_type == 'Computer':
    reference = query_map.get('reference', '').lstrip("=").strip('"')
    return computer_list(reference)
  return None

@app.route('/hatoeas/partitions/<partition_reference>', methods=['GET'])
def hatoeas_partitions(partition_reference):
...
@app.route('/hatoeas/computers/<computer_reference>', methods=['GET'])
def hatoeas_computers(computer_reference):
...
def hatoeas_root():
  return {
    '_links': {
      'raw_search': {
      'href': request.host_url[:-1] + unquoted_url_for('hatoeas', mode='search', query='{query}', select_list='{select_list}')
    },
      'traverse': {
        'href': request.host_url[:-1] + unquoted_url_for('hatoeas', mode='traverse', relative_url='{relative_url}', view='{view}')
      },
    }
  }
...
mode_handlers = {
  None: hatoeas_root,
  'search': hatoeas_search,
  'traverse': hatoeas_traverse,
}

@app.route('/hatoeas', methods=['GET'])
def hatoeas():
