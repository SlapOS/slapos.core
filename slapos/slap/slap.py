...
from hateoas import ConnectionHelper, SlapHateoasNavigator
...
def _syncComputerInformation(func):
  ...
  def decorated(self, *args, **kw):
    ...
    computer = self._getFullComputerInformation(self._computer_id)
...
class Computer(SlapDocument):
  ...
  def _getComputerInformation(self, computer_id):
    xml = self._connection_helper.GET('getComputerInformation', params={'computer_id': computer_id})
    return xml_marshaller.loads(xml)

  def _getFullComputerInformation(self, computer_id):
    """
    ...
    try:
      xml = self._connection_helper.GET(path, params=params)
    except NotFoundError:
      # XXX: This is a ugly way to keep backward compatibility,
      # We should stablise slap library soon.
      xml = self._connection_helper.GET('getComputerInformation', params=params)
