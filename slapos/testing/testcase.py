...
class SlapOSInstanceTestCase(unittest.TestCase):
  ...
  def _cleanup(cls, snapshot_name):
    ...
    try:
      ...
      cls._storeSystemSnapshot("{}._cleanup request destroy".format(snapshot_name))
    try:
      ...
      cls._storeSystemSnapshot("{}._cleanup waitForReport".format(snapshot_name))
    leaked_partitions = [
    ...
    if leaked_partitions:
      ...
      cls._storeSystemSnapshot("{}._cleanup leaked_partitions".format(snapshot_name))
      for cp in leaked_partitions:
        try:
          ...
          cls._storeSystemSnapshot(
              "{}._cleanup leaked_partitions request destruction".format(snapshot_name))
      try:
        cls.slap.waitForReport(max_retry=cls.report_max_retry, debug=cls._debug)
      except:
        cls.logger.exception(
