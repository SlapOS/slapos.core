...
class SlapOSInstanceTestCase(unittest.TestCase):
  ...
  def _copySnapshot(cls, source_file_name, name):
    ...
    if os.path.islink(source_file_name) and not os.path.exists(source_file_name):
      cls.logger.debug("copy broken symlink %s as %s", source_file_name, destination)
      ...
      shutil.copytree(source_file_name, destination)
