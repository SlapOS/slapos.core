...
class TestingSlapOSInstance(object):
  ...
  def __new__(cls):
    if not hasattr(cls, 'slap') or not cls.slap:
      ...
      # TODO partition_base_name ?
      # or create partitions in setupclass and remove them in teardown ?
...
class SlapOSInstanceTestCase(SlapOSTestCase):
  ...
  def getSoftwareURL(cls):
    ...
    Note that software has to be installed on the slapos,
    using installSoftwareUrlList, preferably in setUpModule
    """
  ...
  # Utility methods.
  def getSupervisorRPC(self): # XXX remove ?!
  ...
  def setUpClass(cls):
    ...
    try:
      ...
      try:
        ...
        # TODO check this .args[0] is OK with python 2 & 3
