...
def makeModuleSetUpAndTestCaseClass(
  ...
  def setUpModule():
    ...
    installSoftwareUrlList(cls, [software_url], shared_part_list, debug=debug)

  return setUpModule, SlapOSInstanceTestCase_


def checkSoftware(slap, software_url, shared_part_list):
  # type: (StandaloneSlapOS, str, Iterable[str]) -> None
...
def installSoftwareUrlList(
  ...
  try:
    ...
    for software_url in software_url_list:
      checkSoftware(cls.slap, software_url, shared_part_list)
...
class SlapOSInstanceTestCase(unittest.TestCase):
  ...
  def setUpClass(cls):
    ...
    try:
      ...
      cls._storeSystemSnapshot("{}.setUpClass".format(cls.__name__))
      cls._cleanup("{}.setUpClass".format(cls.__name__))
  ...
  def tearDownClass(cls):
    ...
    """
    cls._cleanup("{}.tearDownClass".format(cls.__name__))
  ...
  @classmethod
  def _cleanup(cls, name=None):
    # type: (Optional[str]) -> None
    """Destroy all instances and stop subsystem.
    Catches and log all exceptions and take snapshot named `name` + the failing step.
    """
    if not name:
      name = cls.__name__
    try:
      ...
      cls._storeSystemSnapshot("{}._cleanup request destroy".format(name))
    try:
      ...
      cls._storeSystemSnapshot("{}._cleanup waitForReport".format(name))
    leaked_partitions = [
    ...
    if leaked_partitions:
      ...
      cls._storeSystemSnapshot("{}._cleanup leaked_partitions".format(name))
      for cp in leaked_partitions:
        try:
          ...
          cls._storeSystemSnapshot(
              "{}._cleanup leaked_partitions request destruction".format(name))
      try:
        cls.slap.waitForReport(max_retry=cls.report_max_retry, debug=cls._debug)
      except:
        cls.logger.exception(
        ...
        cls._storeSystemSnapshot(
            "{}._cleanup leaked_partitions waitForReport".format(name))
    try:
      cls.slap.stop()
    except:
      cls.logger.exception("Error during stop")
      cls._storeSystemSnapshot("{}._cleanup stop".format(name))
