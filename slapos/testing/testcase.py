...
def makeModuleSetUpAndTestCaseClass(
    ...
    debug=os.environ.get('SLAPOS_TEST_DEBUG', False),
    verbose=os.environ.get('SLAPOS_TEST_VERBOSE', None),
):
  ...
  A note about paths:
    ...
    reference. If the class names are long, he trick is then to use a shorter
  ...
  # TODO: handle kill from testnode
  try:
...
class SlapOSInstanceTestCase(unittest.TestCase):
  ...
  def setUpClass(cls):
    ...
    try:
      ...
      # only debug the last.
      if cls.debug and cls.instance_max_retry:
        try:
        ...
        cls.slap.waitForInstance(
            max_retry=cls.instance_max_retry, debug=cls.debug)
  ...
  def _cleanup(cls):
    ...
    try:
      ...
      cls.logger.exception(
          "Error during actual of leaked partitions destruction")
    try:
      cls.slap.stop()
    except:
