...
class TestCliCache(CliMixin):
  ...
  def test_cached_binary(self):
    ...
    self.logger.info.assert_any_call(u' machine compiler target distribution version    id   signature verified compatible? ')
    self.logger.info.assert_any_call(u'-------------------------------------------------------------------------------------')
    self.logger.info.assert_any_call(u'  x86_64                 CentOS Linux 7.5.1804  Core         yes              no     ')
    self.logger.info.assert_any_call(u'  x86_64                    Ubuntu     18.04   bionic        yes              no     ')
