...
class TestCliCache(CliMixin):
  ...
  def test_cached_binary(self):
    ...
    self.logger.info.assert_any_call(u' compiler target distribution version    id   compatible? verified? ')
    self.logger.info.assert_any_call(u'--------------------------------------------------------------------')
    self.logger.info.assert_any_call(u'        ?        CentOS Linux 7.5.1804  Core       no        yes    ')
    self.logger.info.assert_any_call(u'        ?           Ubuntu     18.04   bionic      no        yes    ')
  ...
  def test_uncached_binary(self):
    ...
    self.logger.critical.assert_any_call(
        'Error while looking object %s', 'this_is_uncached_url')
