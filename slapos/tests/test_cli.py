...
class TestCliCache(CliMixin):
  ...
  def test_uncached_binary(self):
    ...
    self.logger.info.assert_any_call(
      'Failed to download from network cache %s: HTTP Error 404: Not Found',
      'this_is_uncached_url')
    self.logger.critical.assert_any_call('Error while looking object %s', 'this_is_uncached_url')
  ...
  def test_bad_cache_dir(self):
    ...
    self.logger.info.assert_any_call(
      'Failed to download from network cache %s: timed out',
      u'https://lab.nexedi.com/nexedi/slapos/raw/1.0.102/software/slaprunner/software.cfg')
