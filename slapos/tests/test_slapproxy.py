...
class TestMigrateVersion10To12(TestInformation, TestRequest, TestSlaveRequest, TestMultiNodeSupport):
  ...
  def test_automatic_migration(self):
    ...
    # Lower level tests
    computer_list = self.db.execute("SELECT * FROM computer13").fetchall()
    self.assertEqual(
        computer_list,
        [(u'computer', u'127.0.0.1', u'255.255.255.255')]
    )

    software_list = self.db.execute("SELECT * FROM software13").fetchall()
    self.assertEqual(
        software_list,
        [(u'/srv/slapgrid//srv//runner/project//slapos/software.cfg', u'computer', u'available')]
    )

    partition_list = self.db.execute("select * from partition13").fetchall()
    self.assertEqual(
        partition_list,
    ...
    slave_list = self.db.execute("select * from slave13").fetchall()
    self.assertEqual(
        slave_list,
        []
    )

    partition_network_list = self.db.execute("select * from partition_network13").fetchall()
    self.assertEqual(
    ...
    table_list = self.db.execute("SELECT name FROM sqlite_master WHERE type='table' ORDER BY name").fetchall()
    self.assertEqual(table_list, [('computer13', ), ('forwarded_partition_request13',), ('partition13', ), ('partition_network13', ), ('slave13', ), ('software13', )])
