...
class TestLocalSoftwareReleaseRootPathMigration(MasterMixin):
  ...
  def checkMoveToSubpath(self, checkUrl):
    initial_url = os.path.join(self._rootdir, 'opt', 'soft', 'software.cfg')
    new_localdir = os.path.join(self._rootdir, 'opt', 'soft')
    checkUrl(initial_url, initial_url, None, localdir=new_localdir)

  def checkMoveToSuperpath(self, checkUrl):
    initial_url = os.path.join(self._rootdir, 'opt', 'soft', 'software.cfg')
    checkUrl(initial_url, initial_url, None, localdir=self._rootdir)

  def test_supply_move_to_subpath(self):
    self.checkMoveToSubpath(self.checkSupplyUrl)

  def test_supply_move_to_superpath(self):
    self.checkMoveToSuperpath(self.checkSupplyUrl)

  def test_request_move_to_subpath(self):
    self.checkMoveToSubpath(self.checkRequestUrl)

  def test_request_move_to_superpath(self):
    self.checkMoveToSuperpath(self.checkRequestUrl)
  ...
  def checkMoveInitialExists(self, checkUrl):
    initial_url = self.createSoftware(self._rootdir)
    new_rootdir = self.newRootDir()
    new_localdir = os.path.join(new_rootdir, 'opt')
    os.mkdir(new_rootdir)
    checkUrl(initial_url, initial_url, new_rootdir, new_localdir)

  def checkMoveBothExist(self, checkUrl):
    initial_url = self.createSoftware(self._rootdir)
    new_rootdir = self.newRootDir()
    expected_url = self.createSoftware(new_rootdir)
    new_localdir = os.path.join(new_rootdir, 'opt')
    checkUrl(initial_url, expected_url, new_rootdir, new_localdir)

  def test_supply_initial_exists(self):
    self.checkMoveInitialExists(self.checkSupplyUrl)

  def test_supply_both_exist(self):
    self.checkMoveBothExist(self.checkSupplyUrl)

  def test_request_initial_exists(self):
    self.checkMoveInitialExists(self.checkRequestUrl)

  def test_request_both_exist(self):
    self.checkMoveBothExist(self.checkRequestUrl)
